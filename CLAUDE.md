...
5. **RBAC por permissões, nunca por nome de papel.** `Permission` é catálogo global e estável do sistema (ex.: `contacts:read`, `deals:write`) — exceção documentada à regra do `workspaceId`. `Role` sempre pertence a um workspace (padrões semeados com `isSystem=true`). O código checa permissões via `@RequirePermissions()`; jamais `if (role.name === 'admin')`.
