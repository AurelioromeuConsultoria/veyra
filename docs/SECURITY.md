...
- **Minimização obrigatória**: `before/after` seguem **allowlist de campos auditáveis por entidade** (ex.: Deal: stage, amount, owner, status). Nunca entram: conteúdo de mensagens/conversas, corpo de anexos, segredos/hashes/tokens, dados clínicos de verticais futuros. Campos fora da allowlist aparecem como `"[changed]"`. A entrada `message` allowlista **apenas** `externalId` (o wamid — identificador do provedor, não conteúdo) e `direction`: é o único ponto de retomada quando perdemos a posse do lease depois de um envio bem-sucedido, e sem ele a trilha do caso incerto era literalmente `{ direction: "[changed]" }`. Corpo de mensagem continua fora, sempre.
