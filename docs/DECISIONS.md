...
**O que NÃO é situação comercial:** o medidor de consumo, inclusive `ai_cost_cents`, fica em `workspace:read`. É o teto que o próprio membro vai esbarrar, e esconder o valor deixaria "limite atingido" sem explicação — o mistério de suporte que este ADR combate. O que exige `billing:manage` é o **contrato**: preço do plano, status da assinatura e fim do período. A distinção é entre _quanto a operação consumiu_ (trabalho) e _quanto e como se paga por isso_ (negócio). Consequência assumida: um Guest vê o gasto de IA do período em dólares. Se isso se mostrar indesejável num piloto real, a mudança é mover `ai_cost_cents` para trás de `billing:manage` e deixar o percentual de uso para todos — decisão de produto, registrada aqui para não ser redescoberta.
