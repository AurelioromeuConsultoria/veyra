...
| `Plan` / `PlanLimit` (globais, catálogo) | key, name, priceCents (**centavos de USD**), **isDefault** (marca TRUE/NULL, unique) / metric, **kind** (counter\|gauge), value (BigInt) | Catálogo global como `Permission`. Limites em LINHAS consultáveis, nunca JSON. `BigInt` porque bytes de storage estouram int32 em 2 GiB. `isDefault` é **load-bearing** desde o ADR-041: define o teto herdado por quem não tem assinatura ativa, e para métrica de custo de terceiro a AUSÊNCIA da linha de limite significa lacuna de configuração, não "ilimitado" |
| `Subscription`                           | planKey, status, currentPeriodStart/End                                                                                                  | Um por workspace; atribuída por **CLI administrativa** (ADR-034). O backfill cobriu os existentes, mas o sistema NÃO pressupõe que exista: sem assinatura ativa, métrica de custo de terceiro herda o teto do plano padrão (ADR-041)                                                                                                                                  |
