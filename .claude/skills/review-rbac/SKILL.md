...
1. **Cobertura de endpoints**: grep por controllers; todo handler tem `@RequirePermissions(...)` ou `@Public()` justificado. Liste os sem decorator — rota autenticada sem permissão declarada é achado Importante; rota pública nova sem justificativa é Bloqueante.
...
8. **Relatório**: achados por severidade (Bloqueante/Importante/Qualidade) com arquivo:linha; veredito final.
